...
D3D11Renderer::D3D11Renderer(int32_t width,
    ...
    : width_(width),
      ...
      use_consumer_leases_(use_consumer_leases) {
...
D3D11Renderer::~D3D11Renderer() {
  if (mailbox_swap_chain_) {
    // Join the completion thread before VideoOutput can be destroyed. Clearing
    // the callback alone cannot recall a callback already copied by that
    // thread.
    mailbox_swap_chain_->Shutdown();
  }
...
D3D11Renderer::ConsumerLease* D3D11Renderer::ConsumerAcquireLease() {
  if (!mailbox_swap_chain_ || !use_consumer_leases_)
    return nullptr;

  auto lease =
      std::unique_ptr<ConsumerLease>(new (std::nothrow) ConsumerLease());
  if (!lease)
    return nullptr;

  MailboxSwapChain::ConsumerFrame frame;
  if (!mailbox_swap_chain_->ConsumerAcquireFrame(&frame))
    return nullptr;

  lease->handle = frame.handle;
  lease->slot = frame.slot;
  lease->submission_id = frame.submission_id;
  lease->mailbox = mailbox_swap_chain_;
  lease->texture = std::move(frame.texture);
  return lease.release();
}

void D3D11Renderer::ReleaseConsumerLease(void* context) {
  std::unique_ptr<ConsumerLease> lease(static_cast<ConsumerLease*>(context));
  if (!lease || !lease->mailbox)
    return;
  lease->mailbox->ConsumerReleaseFrame(lease->slot, lease->submission_id);
}
...
void D3D11Renderer::SetFrameAvailableCallback(std::function<void()> callback) {
  if (mailbox_swap_chain_) {
...
void D3D11Renderer::SetGPUThreadPriority(int priority) {
  ...
  Microsoft::WRL::ComPtr<IDXGIDevice> dxgi_device;
  if (d3d_11_device_ && SUCCEEDED(d3d_11_device_.As(&dxgi_device)) &&
      dxgi_device) {
    media_kit_video::ApplyGpuThreadPriority(dxgi_device.Get());
  }
}
...
bool D3D11Renderer::CreateFrameBridge() {
  if (output_mode_ == media_kit_video::VideoOutputMode::kFixedHandleCopy) {
    ...
    const HRESULT hr = FixedHandleTextureBridge::Create(d3d_11_device_.Get(),
                                                        width_, height_, &raw);
    if (FAILED(hr)) {
  ...
  const HRESULT hr =
      MailboxSwapChain::Create(d3d_11_device_.Get(), width_, height_,
                               non_blocking, use_consumer_leases_, &raw);
  if (FAILED(hr)) {
