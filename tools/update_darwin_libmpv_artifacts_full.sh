...
MEDIA_KIT_DIR="$(cd "${SCRIPT_DIR}/.." && pwd)"
cd "${MEDIA_KIT_DIR}"

REPO="ha1o0/libmpv-darwin-build"
DRY_RUN=0
VERSION="v2.19.4"
VERSION_SET=0
...
usage() {
  cat <<'EOF'
Usage:
  tools/update_darwin_libmpv_artifacts_full.sh [version] [--dry-run] [--repo=<owner/name>]

Example:
  tools/update_darwin_libmpv_artifacts_full.sh
  tools/update_darwin_libmpv_artifacts_full.sh v2.19.4
...
echo "Fetching release data for ${REPO}@${VERSION}..."
curl -fsSL \
...
update_makefile() {
  ...
  if [[ "$DRY_RUN" -eq 1 ]]; then
    echo "Would update ${path} -> version=${version}, checksum=${checksum}"
    return
  fi
